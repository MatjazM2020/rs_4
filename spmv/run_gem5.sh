...
if [[ -n "${SLURM_SUBMIT_DIR:-}" && -d "$SLURM_SUBMIT_DIR/spmv" ]]; then
    SCRIPT_DIR=$SLURM_SUBMIT_DIR/spmv
elif [[ -n "${SLURM_SUBMIT_DIR:-}" && -f "$SLURM_SUBMIT_DIR/spmv.cpp" ]]; then
...
if [[ ! -f "$BIN_ROOT/spmv.bin" ]]; then
    echo "Missing SpMV binary. Run ./make_apptainer.sh before this script." >&2
    exit 1
fi
